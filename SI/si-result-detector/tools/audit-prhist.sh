...
JAR_PATH=${SI_RESULT_DETECTOR_JAR:-${PREDICATE_SI_JAR:-${POLYSI_JAR:-"$PROJECT_DIR/build/libs/si-result-detector-1.0.0-SNAPSHOT.jar"}}}
MONOSAT_NATIVE_DIR=${MONOSAT_NATIVE_DIR:-"$PROJECT_DIR/build/monosat"}
HEAP_SIZE=${SI_RESULT_DETECTOR_HEAP:-${PREDICATE_SI_HEAP:-${POLYSI_HEAP:-8g}}}
OUTPUT_ROOT=${SI_RESULT_DETECTOR_OUTPUT_DIR:-${PREDICATE_SI_OUTPUT_DIR:-${POLYSI_OUTPUT_DIR:-"/tmp/si-result-detector-prhist-audit"}}}
...
Environment variables:
  SI_RESULT_DETECTOR_JAR         Override the detector jar path.
  MONOSAT_NATIVE_DIR              Directory containing libmonosat.so.
  SI_RESULT_DETECTOR_HEAP        JVM heap size passed via -Xmx. Default: 8g
  SI_RESULT_DETECTOR_JAVA_OPTS   Extra JVM options, split on spaces.
...
if [[ ! -f "$JAR_PATH" ]]; then
  echo "si-result-detector jar not found: $JAR_PATH" >&2
  exit 2
fi
...
JAVA_OPTS=${SI_RESULT_DETECTOR_JAVA_OPTS:-${PREDICATE_SI_JAVA_OPTS:-${POLYSI_JAVA_OPTS:-}}}
if [[ -n "$JAVA_OPTS" ]]; then
