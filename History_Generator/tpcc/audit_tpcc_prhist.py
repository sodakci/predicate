#!/usr/bin/env python3
"""Audit a generated BenchBase TPC-C PRHIST case without trusting its manifest."""
...
from tpcc_trace_to_prhist import ConversionError, load_jsonl, validate_structure
...
def audit(case_dir: Path) -> dict:
    ...
    raw_path = case_dir / "raw_tpcc_trace.jsonl"
    if raw_path.is_file():
        ...
        if len(raw_initial) != stats["initial_keys"]:
            raise ConversionError(
                f"raw trace initial count {len(raw_initial)} != PRHIST initial count {stats['initial_keys']}"
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate source versions, session order, and StockLevel membership for a TPC-C PRHIST case.")
