...
public abstract class Worker<T extends BenchmarkModule> implements Runnable {
      ...
      while (retryCount < maxRetryCount && this.workloadState.getGlobalState() != State.DONE) {
        ...
        if (this.conn == null) {
          try {
            if (!this.configuration.getNewConnectionPerTxn()) {
              if (retryCount > 0) {
                long delaySeconds = Math.min(retryCount, 5);
                LOG.info("Backing off {} seconds before reconnecting.", delaySeconds);
                try {
        ...
        try {
          ...
          conn.commit();
          this.afterTransactionCommit(conn, transactionType);
          ...
          if (status != TransactionStatus.SUCCESS
              ...
              && conn != null) {
            this.afterTransactionAbort(conn, transactionType, status);
          }
  ...
  /**
   * Optional instrumentation hook invoked after the business transaction has committed.
   * Implementations must persist any bookkeeping themselves.
   */
  protected void afterTransactionCommit(Connection conn, TransactionType txnType) {}

  /**
   * Optional instrumentation hook invoked after an unsuccessful transaction attempt. The connection
   * can be unavailable after a connection failure.
   */
